# NH-Data Deployment Checklist

## Before GitHub Push

- Production domain is currently set to `https://yoyangwon.com/` in canonical URLs and sitemap generation.
...
- `https://yoyangwon.com/`, `https://www.yoyangwon.com/`, and `sc-domain:yoyangwon.com` can be added to the connected GSC account, but currently return `siteUnverifiedUser`.
...
- The live `yoyangwon.com` domain does not currently serve this local static site, so adding verification files locally is not enough; the file must be deployed to the live host first.
- The live `https://yoyangwon.com/sitemap.xml` currently contains only `/lander`; it must be replaced by the generated sitemap before GSC submission can show the intended URLs.
